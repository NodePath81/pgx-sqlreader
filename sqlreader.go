...
// SQLReaderConfig is the configuration for SQLReader.
type SQLReaderConfig struct {
	// LogLevel is the level of logging to use.
	LogLevel LogLevel
	// MetricsConfig is the configuration for metrics collection.
	MetricsConfig MetricsConfig
}

// DefaultSQLReaderConfig provides default configuration for SQLReader.
var DefaultSQLReaderConfig = SQLReaderConfig{
...
func New(queriesFS embed.FS, queriesDir, migrationsDir string) (*SQLReader, error) {
	return NewWithConfig(queriesFS, queriesDir, migrationsDir, DefaultSQLReaderConfig)
}

// NewWithConfig creates a new SQLReader instance with the provided configuration.
//
// Parameters:
//   - queriesFS: An embedded filesystem containing SQL queries and migrations
//   - queriesDir: The directory in the filesystem containing SQL query files
//   - migrationsDir: The directory in the filesystem containing migration files
//   - config: Configuration options for the SQLReader
//
// Returns a new SQLReader instance or an error if initialization fails.
func NewWithConfig(queriesFS embed.FS, queriesDir, migrationsDir string, config SQLReaderConfig) (*SQLReader, error) {
	logger := NewLogger(config.LogLevel)
	metrics := NewMetricsCollector(config.MetricsConfig)

	logger.Debug("Initializing SQLReader",
		"queriesDir", queriesDir,
		"migrationsDir", migrationsDir)

	queries, err := newQueryStore(queriesFS, queriesDir)
	if err != nil {
		logger.Error("Failed to initialize query store", "error", err)
		metrics.IncrementError("init_query_store")
		return nil, fmt.Errorf("initializing query store: %w", err)
	}

	logger.Info("SQLReader initialized successfully",
		"queryCount", len(queries.queries))
...
func (r *SQLReader) ConnectPool(pool *pgxpool.Pool) *Connector {
	loader := &queryLoader{
		db:      pool,
		querier: r.queries,
		logger:  r.logger.With("connection", "pool"),
		metrics: r.metrics,
	}

	r.logger.Debug("Created database pool connector")
...
func (r *SQLReader) ConnectTx(tx pgx.Tx) *Connector {
	loader := &queryLoader{
		db:      tx,
		querier: r.queries,
		logger:  r.logger.With("connection", "transaction"),
		metrics: r.metrics,
	}

	r.logger.Debug("Created transaction connector")
...
func (c *Connector) Exec(ctx context.Context, name string, args ...interface{}) error {
	ctx = ContextWithLogger(ctx, c.reader.logger)
	ctx = ContextWithMetrics(ctx, c.reader.metrics)

	ctx, done := TrackDuration(ctx, "exec")
	err := c.loader.exec(ctx, name, args...)
	done(err == nil)
...
func (c *Connector) QueryRow(ctx context.Context, name string, scanner func(pgx.Row) error, args ...interface{}) error {
	...
	ctx, done := TrackDuration(ctx, "queryRow")
	err := c.loader.queryRow(ctx, name, scanner, args...)
	done(err == nil)
...
func (c *Connector) QueryRows(ctx context.Context, name string, scanner func(pgx.Rows) error, args ...interface{}) error {
	...
	ctx, done := TrackDuration(ctx, "queryRows")
	err := c.loader.queryRows(ctx, name, scanner, args...)
	done(err == nil)
...
func (c *Connector) InitiateMigration(ctx context.Context) error {
	...
	c.reader.logger.Debug("Initializing migration manager")

	c.reader.migrations = newMigrationManager(c.db, c.reader.queriesFS, c.reader.migrationsDir)
	c.reader.migrations.logger = c.reader.logger.With("component", "migration_manager")
	c.reader.migrations.metrics = c.reader.metrics

	ctx, done := TrackDuration(ctx, "init_migration")
	err := c.reader.migrations.Initialize(ctx)
	done(err == nil)

	if err != nil {
		c.reader.logger.Error("Failed to initialize migrations table", "error", err)
	} else {
		c.reader.logger.Debug("Migrations table initialized successfully")
	}
...
func (c *Connector) Migrate(ctx context.Context) error {
	...
	if isTx {
		...
		ctx, done := TrackDuration(ctx, "migrate")
		err := c.reader.migrations.Migrate(ctx)
		done(err == nil)

		if err != nil {
			logger.Error("Migration failed", "error", err, "duration", time.Since(startTime))
		} else {
			logger.Info("Migration completed successfully", "duration", time.Since(startTime))
		}
	...
	logger.Debug("Starting new transaction for migration")

	conn, ok := c.db.(*pgxpool.Pool)
	if !ok {
		logger.Error("Unexpected connection type", "expected", "*pgxpool.Pool")
	...
	txMigrations.logger = c.reader.logger.With("component", "migration_manager", "connection", "transaction")
	txMigrations.metrics = c.reader.metrics

	// Apply migrations
	ctx, done := TrackDuration(ctx, "migrate")
	err = txMigrations.Migrate(ctx)
	done(err == nil)

	if err != nil {
		logger.Error("Migration failed, rolling back transaction", "error", err)
		tx.Rollback(ctx)
		return err
	}

	// Commit the transaction
	logger.Debug("Migration successful, committing transaction")
...
func (c *Connector) Rollback(ctx context.Context) error {
	...
	if isTx {
		...
		logger.Debug("Using existing transaction for rollback")

		ctx, done := TrackDuration(ctx, "rollback")
		err := c.reader.migrations.Rollback(ctx)
		done(err == nil)

		if err != nil {
			logger.Error("Rollback failed", "error", err, "duration", time.Since(startTime))
		} else {
			logger.Info("Rollback completed successfully", "duration", time.Since(startTime))
		}
	...
	logger.Debug("Starting new transaction for rollback")
	...
	// Apply rollback
	ctx, done := TrackDuration(ctx, "rollback")
	err = txMigrations.Rollback(ctx)
	done(err == nil)

	if err != nil {
		logger.Error("Rollback failed, rolling back transaction", "error", err)
		tx.Rollback(ctx)
		return err
	}

	// Commit the transaction
	logger.Debug("Rollback successful, committing transaction")
	if err := tx.Commit(ctx); err != nil {
...
func (c *Connector) ExecuteJSONBQuery(ctx context.Context, name string, scanner func(pgx.Rows) error, args ...interface{}) error {
	return c.QueryRows(ctx, name, scanner, args...)
}
